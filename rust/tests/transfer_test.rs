...
/// The test node panics when it serves concurrent reading calls, so the
/// end-to-end tests take turns instead of running in parallel.
static CHAIN: Mutex<()> = Mutex::const_new(());
...
async fn test_transfer_asset() {
    ...
    // Events arrive asynchronously, so an empty channel here is not a failure.
    let _ = rx.try_recv();
}

/// Same flow as above, but signed with a Secp256k1 key, to prove the chain
/// accepts the secp256k1 pubkey/address/signature encoding.
#[tokio::test]
async fn test_transfer_asset_secp256k1() {
    let _guard = CHAIN.lock().await;

    let kp = KeyPair::generate_secp256k1();
    let to_kp = KeyPair::generate_secp256k1();
    ...
    const CREATE_AMOUNT: u64 = 300;
    const TRANSFER: u64 = 120;
    ...
    client
        .write_chain(
            "asset",
            "Transfer",
            &serde_json::json!({ "to": to_addr, "amount": TRANSFER }),
            0,
            0,
            0,
        )
        .await
        .expect("write_chain Transfer failed");
    tokio::time::sleep(Duration::from_secs(6)).await;
    assert_eq!(
        query_balance(&client, &my_addr).await,
        CREATE_AMOUNT - TRANSFER
    );
    assert_eq!(query_balance(&client, &to_addr).await, TRANSFER);
}
