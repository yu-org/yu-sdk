...
- Sr25519: `31` (ASCII `'1'`)
- Ed25519: `32` (ASCII `'2'`)
- Secp256k1: `33` (ASCII `'3'`)

For Secp256k1, the signature is ECDSA over `SHA256(msg)`, serialized as a
64-byte `R || S` in lower-S form — matching tendermint's `PrivKey.Sign`
(the `msg` passed to `sign()` is already the 32-byte `BytesToHash` result,
so this is an extra hash on top of that). The address is
`RIPEMD160(SHA256(compressed_pubkey))`. These vectors are checked against
ones generated by yu itself in `tests/test_keypair.py`, which needs no
running chain.
